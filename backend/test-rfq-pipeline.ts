/**
 * RFQ Pipeline Test Script
 * Tests regex extraction → LLM fallback pipeline with generated test emails
 * No database modifications, pure in-memory testing
 */
...
class RFQExtractorTester {
  ...
  testRegexExtraction(email: TestEmail): Promise<ExtractionResult> {
  ...
  async testLlmExtraction(email: TestEmail): Promise<ExtractionResult> {
    try {
      const prompt = [
  ...
  async testPipeline(email: TestEmail): Promise<{
    ...
    let llm: ExtractionResult = { items: [], confidence: 0, source: 'llm' };

    // If regex confidence < 80%, try LLM fallback
    ...
    const itemsAccuracy = Math.min(
      (itemsExtracted / email.expectedItems) * 100,
      100,
    );
    const confidenceBonus = bestResult.confidence;
    const satisfaction = Math.round(
      itemsAccuracy * 0.7 + confidenceBonus * 0.3,
    );
...
async function main() {
  ...
  for (let i = 0; i < testEmails.length; i++) {
    ...
    try {
      ...
      console.log(`\n📊 PIPELINE RESULTS:`);
      console.log(
        `  Regex Extraction: ${result.regex.items.length} items @ ${result.regex.confidence}% confidence`,
      );
      ...
      if (result.llm.items.length > 0) {
        console.log(
          `  LLM Fallback: ${result.llm.items.length} items @ ${result.llm.confidence}% confidence`,
        );
      ...
      const best =
        result.llm.items.length > result.regex.items.length
          ? result.llm
          : result.regex;
      console.log(`\n✨ EXTRACTED OUTPUT:`);
      console.log(`  Source: ${best.source.toUpperCase()}`);
      console.log(
        `  Items Extracted: ${best.items.length}/${email.expectedItems}`,
      );
      best.items.forEach((item) => {
        console.log(
          `    ✓ ${item.product_name} | Qty: ${item.quantity} | Unit: ${item.unit}`,
        );
      });
