...
export class AccountingIntegrationsService {
  ...
  private ensureXeroConfigured() {
    ...
    if (!clientId || !clientSecret || !tenantId || !redirectUri) {
      throw new BadRequestException('Xero integration is not configured');
    }
  ...
  private ensureQuickBooksConfigured() {
    ...
    if (!clientId || !clientSecret || !realmId || !redirectUri) {
      throw new BadRequestException('QuickBooks integration is not configured');
    }
  ...
  private parseTokenResponse(payload: unknown) {
  ...
  async getStatus(tenantId: string) {
    ...
    const statusByProvider = rows.reduce<
      Partial<Record<'xero' | 'quickbooks', AccountingIntegrationStatusRow>>
    >((acc, row) => {
  ...
  async handleXeroCallback(code: string, state?: string) {
    ...
    if (!accessToken) {
      throw new BadRequestException('Xero token response missing access token');
    }
    const expiresAt = tokenJson.expires_in
      ? new Date(Date.now() + tokenJson.expires_in * 1000)
      : undefined;

    const encRefresh = encryptToken(tokenJson.refresh_token);
  ...
  async handleQuickBooksCallback(
    ...
    if (!tokenResponse.ok) {
      throw new BadRequestException('QuickBooks token exchange failed');
    }
    ...
    if (!accessToken) {
      throw new BadRequestException(
        'QuickBooks token response missing access token',
      );
    }
    const expiresAt = tokenJson.expires_in
      ? new Date(Date.now() + tokenJson.expires_in * 1000)
    ...
    const encRefresh2 = encryptToken(tokenJson.refresh_token);
    await this.prisma.accountingIntegration.upsert({
      where: {
        tenant_id_provider: {
          tenant_id: payload.tenantId,
  ...
  async exportInvoiceToXero(tenantId: string, invoiceId: string) {
    this.ensureXeroConfigured();
    ...
    if (!integration) {
      throw new BadRequestException('Xero integration not connected');
    }
  ...
  async exportInvoiceToQuickBooks(tenantId: string, invoiceId: string) {
    this.ensureQuickBooksConfigured();
    ...
    if (!integration) {
      throw new BadRequestException('QuickBooks integration not connected');
    }
    ...
    const realmId =
      integration.realm_id || process.env.QUICKBOOKS_REALM_ID || '';
    ...
    const responseJson = (await response
      .json()
      .catch(() => ({}))) as QuickBooksInvoiceResponse;
    const status = response.ok ? 'success' : 'failed';
