...
export class SettingsService {
  ...
  async getNamespace(tenantId: string, namespace: string) {
    if (!tenantId || !namespace)
      throw new BadRequestException('tenantId and namespace required');
  ...
  async upsertNamespace(
    ...
    // Record before state
    const existingRows = await this.prisma.setting.findMany({
      where: { tenant_id: tenantId, namespace },
    });
    ...
    for (const key of keys) {
      ...
      await this.prisma.setting.upsert({
        where: {
          tenant_id_namespace_key: { tenant_id: tenantId, namespace, key },
        },
        update: { value },
  ...
  private loadValidatorForNamespace(
    namespace: string,
  ...
  async updateCompany(
    ...
    const before = await this.prisma.settingsCompany.findUnique({
      where: { tenant_id: tenantId },
    });
  ...
  async updateNotifications(
    ...
    const before = await this.prisma.settingsNotifications.findUnique({
      where: { tenant_id: tenantId },
    });
    const result = await this.prisma.settingsNotifications.upsert({
      where: { tenant_id: tenantId },
      update: body,
      create: {
        tenant_id: tenantId,
