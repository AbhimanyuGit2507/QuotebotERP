...
const makePrismaMock = () => ({
  ...
  settingsTemplate: { findMany: jest.fn(), upsert: jest.fn(), updateMany: jest.fn(), deleteMany: jest.fn() },
  automationRule: { findMany: jest.fn(), create: jest.fn(), updateMany: jest.fn(), deleteMany: jest.fn() },
});
...
describe('SettingsService', () => {
  ...
  beforeEach(() => {
    ...
    service = new SettingsService(prismaMock as any, auditMock as any);
  });
  ...
  it('upsertNamespace accepts valid item_match.config payload', async () => {
    ...
    const res = await service.upsertNamespace('tenant1', 'item_match.config', payload);
  ...
  it('upsertNamespace rejects invalid item_match.config payload', async () => {
    ...
    await expect(service.upsertNamespace('tenant1', 'item_match.config', payload)).rejects.toThrow(BadRequestException);
  });
});
