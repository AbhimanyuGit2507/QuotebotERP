...
async function waitFor(producer, options) {
    ...
    console.log(`[timeout] ${options.stepName} did not complete in ${options.timeoutMs}ms`);
    return null;
}
async function gmailFindSentMessage(accessToken, subject, expectedRecipient) {
    ...
    if (!listResponse.ok) {
        ...
        throw new Error(`Gmail sent-list check failed (${listResponse.status}): ${errorBody}`);
    }
    ...
    for (const msg of messages) {
        const detailResponse = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${msg.id}?format=metadata&metadataHeaders=To&metadataHeaders=Subject`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${accessToken}`,
            },
        ...
        const toHeader = headers.find((h) => String(h.name || '').toLowerCase() === 'to')?.value ||
            '';
        const subjectHeader = headers.find((h) => String(h.name || '').toLowerCase() === 'subject')
            ?.value || '';
        if (normalizeText(subjectHeader).includes(normalizeText(subject)) &&
...
async function main() {
    ...
    try {
        ...
        if (!emailAccount) {
            throw new Error('No active Gmail email account found. Connect Gmail before running this test.');
        }
        ...
        const processedMessage = await waitFor(async () => {
            ...
            if (message.processing_status !== 'parsed')
                return null;
            const payload = message.raw_payload && typeof message.raw_payload === 'object'
                ? message.raw_payload
                : {};
        ...
        const payload = processedMessage.raw_payload &&
            typeof processedMessage.raw_payload === 'object'
            ? processedMessage.raw_payload
            : {};
        const autoRfqId = typeof payload.rfq_id === 'string' && payload.rfq_id.trim().length > 0
            ? payload.rfq_id
            : null;
        const autoQuotationId = typeof payload.quotation_id === 'string' &&
        ...
        const extractedCoveragePerfect = rfq.items.length >= 2 &&
            Array.from(expectedByProduct.keys()).every((name) => rfq.items.some((item) => normalizeText(item.product_name) === name));
        assertions.push({
        ...
        const sendResult = await quotationsService.sendByEmail(manualQuotation.id, tenant.id, {
            email_account_id: emailAccount.id,
            to: [buyerEmail],
        ...
        if (!outbound || outbound.status !== 'sent') {
            throw new Error(`Outbound email was not sent. status=${outbound?.status || 'missing'} error=${outbound?.last_error || ''}`);
        }
        const refreshedToken = await emailService.refreshEmailAccountAccessToken(emailAccount.id, tenant.id);
        ...
        const gmailMessage = await gmailFindSentMessage(refreshedToken.access_token, `Quotation ${manualQuotation.number}`, buyerEmail);
        assertions.push({
        ...
        for (const item of assertions) {
            console.log(`${item.passed ? 'PASS' : 'FAIL'} | ${item.name}${item.details ? ` | ${item.details}` : ''}`);
        }
...
void main().catch((error) => {
    console.error('\nAutonomous E2E flow failed:', error instanceof Error ? error.message : String(error));
    process.exit(1);
});
//# sourceMappingURL=test-autonomous-email-rfq-quotation-e2e.js.map