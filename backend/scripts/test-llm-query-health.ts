...
type Provider =
  ...
  | 'deepseek'
  | 'openrouter';
type QueryMode = 'classifier' | 'extractor' | 'both';
type CerebrasModel =
  | 'gpt-oss-120b'
  | 'llama3.1-8b'
...
type GroqConfig = ChatConfig & { model: string };
type CerebrasConfig = ChatConfig & { defaultModel: string };
type GeminiConfig = ChatConfig & { model: string };
type TogetherConfig = ChatConfig & { model: string };
type MistralConfig = ChatConfig & { model: string };
type DeepSeekConfig = ChatConfig & { model: string };
type OpenRouterConfig = ChatConfig & { model: string };

const QUERY_MODE = (process.env.LLM_QUERY_HEALTH_MODE || 'both').toLowerCase() as QueryMode;
...
function getModeList(): Array<'classifier' | 'extractor'> {
  if (QUERY_MODE === 'classifier') return ['classifier'];
  if (QUERY_MODE === 'extractor') return ['extractor'];
  return ['classifier', 'extractor'];
}

function promptFor(query: 'classifier' | 'extractor') {
...
function getCerebrasConfig(): CerebrasConfig {
  return {
...
function getTogetherConfig(): TogetherConfig {
  return {
    apiKey: process.env.TOGETHER_API_KEY || '',
    baseUrl: process.env.TOGETHER_BASE_URL || 'https://api.together.xyz/v1',
    model:
      process.env.TOGETHER_MODEL || 'mistralai/Mistral-Nemo-Instruct-2407',
  };
}
...
function isValidClassifierResponse(parsed: unknown): boolean {
  return (
    Boolean(parsed) &&
    typeof parsed === 'object' &&
    Array.isArray((parsed as Record<string, unknown>).rfq_ids) &&
    Array.isArray((parsed as Record<string, unknown>).non_rfq_ids)
  );
}

function isValidExtractorResponse(parsed: unknown): boolean {
  return (
    Boolean(parsed) &&
    typeof parsed === 'object' &&
    typeof (parsed as Record<string, unknown>).is_rfq === 'boolean' &&
    Array.isArray((parsed as Record<string, unknown>).items)
  );
}

function buildStrictJsonMessages(prompt: string) {
  return [
    {
      role: 'system' as const,
      content:
        'You are a strict JSON generator. Return exactly one valid JSON object and nothing else.',
    },
...
async function callOpenRouterCompletion(prompt: string) {
...
async function callGeminiCompletion(prompt: string) {
  const config = getGeminiConfig();

  const response = await fetch(
    `${config.baseUrl}/${encodeURIComponent(config.model)}:generateContent?key=${encodeURIComponent(config.apiKey)}`,
    {
      method: 'POST',
...
function buildRepairPrompt(query: 'classifier' | 'extractor', originalPrompt: string, badResponse: string) {
  return [
...
async function runChatQuery(
  ...
  const repairedValid =
    query === 'classifier' ? isValidClassifierResponse(repairedParsed) : isValidExtractorResponse(repairedParsed);
...
async function runOpenRouterQuery(
  query: 'classifier' | 'extractor',
  prompt: string,
): Promise<QueryResult> {
  const config = getOpenRouterConfig();
...
async function runGeminiQuery(query: 'classifier' | 'extractor', prompt: string): Promise<QueryResult> {
  const config = getGeminiConfig();
...
async function main() {
  ...
  for (const provider of PROVIDER_LIST) {
    ...
    if (provider === 'groq') {
      ...
      for (const query of getModeList()) {
        const result: QueryResult = config.apiKey
    ...
    for (const query of getModeList()) {
      const result = config.apiKey
        ...
        : { provider: 'gemini' as const, model: config.model, query, ok: false, text: '', error: 'Missing GEMINI_API_KEY' };
      results.push(result);
      printResult(result);
    }
  }
